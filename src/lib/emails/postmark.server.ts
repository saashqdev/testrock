...
import { InboundEmailDetailsDto } from "@/lib/dtos/email/InboundEmailDetailsDto";
import { InboundEmailDto } from "@/lib/dtos/email/InboundEmailDto";
...
async function getBaseTemplateModel() {
  const currentTenantUrl = getBaseURL()?.toString();

...
export async function storeRowMediaInStorageProvider(entity: Entity, row: RowWithDetailsDto | null) {
  if (!row) {
    return;
  }
              ...
              if (process.env.SUPABASE_API_URL && process.env.SUPABASE_KEY) {
                try {
                  ...
                  if (createdFile.publicUrl) {
                    ...
                    return await db.rows.updateRowMedia(media.id, {
                      file: "",
...
export async function deleteRowMediaFromStorageProvider(row: RowWithDetailsDto | null) {
  if (!row) {
    return;
  }
  const mediaRowValues = row.values.filter((f) => f.media && f.media.length > 0);

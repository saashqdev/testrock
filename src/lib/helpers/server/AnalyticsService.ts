...
async function getOrCreateUniqueVisitor({
  ...
  const visitor = await cachified({
    ...
    ttl: 60 * 60,
    getFreshValue: async () => {
      return prisma.analyticsUniqueVisitor.findUnique({
        where: {
          cookie,
        },
      });
  ...
  return await prisma.analyticsUniqueVisitor
    .create({
      data: {
        cookie,
...
async function getAnalyticsOverview({ withUsers, period, portalId }: { withUsers: boolean; period: PeriodFilter; portalId: string | null | undefined }) {
  ...
  if (period === "all-time") {
    const firstDate = await prisma.analyticsUniqueVisitor.findFirst({
      where: { portalId },
  ...
  const uniqueVisitors = await prisma.analyticsUniqueVisitor.count({
  ...
  const liveVisitors = await prisma.analyticsPageView.groupBy({
    by: ["uniqueVisitorId"],
    where: {
      portalId,
      createdAt: {
  ...
  let topUrls = (
    await prisma.analyticsPageView.groupBy({
      by: ["url"],
      _count: true,
      where: { portalId, createdAt },
    })
  ...
  const topOs = (await prisma.analyticsUniqueVisitor.groupBy({ by: ["os"], _count: true, where: { portalId, createdAt } })).map((f) => {
    return { name: f.os, count: f._count };
  });
  const topDevices = (await prisma.analyticsUniqueVisitor.groupBy({ by: ["device"], _count: true, where: { portalId, createdAt } })).map((f) => {
    return { name: f.device, count: f._count };
  });
  const topCountries = (await prisma.analyticsUniqueVisitor.groupBy({ by: ["country"], _count: true, where: { portalId, createdAt } })).map((f) => {
    return { name: f.country, count: f._count };
  });
  const topVia = (await prisma.analyticsUniqueVisitor.groupBy({ by: ["via"], _count: true, where: { portalId, createdAt } })).map((f) => {
    return { name: f.via, count: f._count };
  });
  const topMedium = (await prisma.analyticsUniqueVisitor.groupBy({ by: ["medium"], _count: true, where: { portalId, createdAt } })).map((f) => {
    return { name: f.medium, count: f._count };
  });
  const topCampaign = (await prisma.analyticsUniqueVisitor.groupBy({ by: ["campaign"], _count: true, where: { portalId, createdAt } })).map((f) => {
  ...
  if (withUsers) {
    topUserIds = (await prisma.analyticsUniqueVisitor.groupBy({ by: ["userId"], _count: true, where: { portalId, createdAt } })).map((f) => {
      return { id: f.userId ?? "", count: f._count };
    });
  }
  const users = await db.users.adminGetAllUsersNamesInIds(topUserIds.map((f) => f.id));
...
async function getCharts({ period, portalId }: { period: PeriodFilter; portalId: string | null | undefined }) {
  ...
  const pageViewsByDay = await prisma.analyticsPageView.groupBy({
    by: ["createdAt"],
    _count: true,
    where: { portalId, createdAt },
  });

  const uniqueVisitorsByDay = await prisma.analyticsUniqueVisitor.groupBy({
    by: ["createdAt"],
    _count: true,
    where: { portalId, createdAt },
  });
  ...
  const toDateString = (date: Date) => {
    ...
    return DateUtils.dateMonthDayYear(date);
  };
...
async function getPageViews(filters: {
  ...
  return await prisma.analyticsPageView.findMany({
    where: {
      OR: [
...
async function getFromRequest({ appConfiguration, request, userId }: { appConfiguration: AppConfigurationDto; request: Request; userId: string | null }) {

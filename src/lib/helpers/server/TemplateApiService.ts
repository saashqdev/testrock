...
async function fetchRowWithRelationships({
  ...
  backlog?: { rowId: string; relation: "parent" | "child"; attachTo: RowWithDetailsDto }[];
...
async function getRowInApiFormatWithRecursiveRelationships({
  ...
  return {
    ...
    entityId: entity.id,
    tenant: row.tenantId ? await db.tenants.getTenant(row.tenantId) : null,
    data: {
...
function pruneDuplicateRows(mainRow: RowWithDetailsDto): void {
  const seenRowIds: Set<string> = new Set();

  let currentLevelRows: RowWithDetailsDto[] = [mainRow];
  while (currentLevelRows.length > 0) {
    const nextLevelRows: RowWithDetailsDto[] = [];
      ...
      if (row.parentRows) {
        for (let i = row.parentRows.length - 1; i >= 0; i--) {
          ...
          if (seenRowIds.has(parent.id)) {
            ...
            nextLevelRows.push(parent as RowWithDetailsDto);
          }
        }
      }
      ...
      if (row.childRows) {
        for (let i = row.childRows.length - 1; i >= 0; i--) {
          ...
          if (seenRowIds.has(child.id)) {
            ...
            nextLevelRows.push(child as RowWithDetailsDto);
          }
        }
      }

...
export async function createPlan(
  ...
  // Save to db
  const product = await db.subscriptionProducts.createSubscriptionProduct({
  ...
  await Promise.all(
    features
      ...
      .map(async (feature) => {
        // Save to db
        return await db.subscriptionProducts.createSubscriptionFeature(product.id, feature);
      })
  );
  ...
  if (usageBasedPrices) {
    await Promise.all(
      usageBasedPrices.map(async (usageBasedPrice) => {
        ...
        const createdPrice = await db.subscriptionProducts.createSubscriptionUsageBasedPrice({
          subscriptionProductId: product.id,
          stripeId: stripePrice?.id ?? "",
...
export async function updatePlan(plan: SubscriptionProductDto, features: SubscriptionFeatureDto[]) {
  ...
  return await Promise.all(
    features
      .sort((a, b) => a.order - b.order)
      .map(async (feature) => {
        return await db.subscriptionProducts.createSubscriptionFeature(plan.id ?? "", feature);
      })
  );
}
...
export async function addTenantProductsFromCheckoutSession({
  ...
  if (!checkoutSession) {
    ...
    await db.checkoutSessions.updateCheckoutSessionStatus(checkoutSession.id, {
      pending: false,
      createdUserId,
      createdTenantId,
    });

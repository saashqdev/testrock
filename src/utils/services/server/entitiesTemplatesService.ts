...
export async function validateEntitiesFromTemplate(template: EntitiesTemplateDto) {
  ...
  await Promise.all(
    template.entities.map(async (entity) => {
      const existingEntity = await db.entities.findEntityByName({ tenantId: null, name: entity.name });
      if (existingEntity) {
...
export async function importEntitiesFromTemplate({ template, createdByUserId }: { template: EntitiesTemplateDto; createdByUserId: string }) {
  const maxEntityOrder = await db.entities.getMaxEntityOrder();
  const createdEntities = await Promise.all(
    template.entities.map(async (entity, idx) => {
      const createdEntity = await db.entities.createEntity({
        name: entity.name,
        slug: entity.slug,
      ...
      await db.permissions.createEntityPermissions(createdEntity);

      const newEntity = await db.entities.getEntityByName({ tenantId: null, name: createdEntity.name });
      if (!newEntity) {
        throw new Error("Unable to fetch newly created entity: " + createdEntity.name);
      }
      ...
      const properties = await Promise.all(
        entity.properties.map(async (property, idxProperty) => {
          ...
          const createdProperty = await db.properties.createProperty({
          ...
          if (property.options) {
            await db.properties.updatePropertyOptions(
              createdProperty.id,
          ...
          if (property.attributes) {
            await db.properties.updatePropertyAttributes(
              createdProperty.id,
      ...
      if (entity.views) {
        await Promise.all(
          entity.views.map(async (view, idx) => {
            ...
            let viewProperties: { propertyId: string | null; order: number; name: string }[] = view.properties
              ? await Promise.all(
                  view.properties.map(async (prop, idxProp) => {
                    const property = await db.properties.getEntityPropertyByName(createdEntity.id, prop);
                    return {
                      order: idxProp + 1,
            ...
            await db.entityViews.updateEntityViewProperties(entityView.id, viewProperties);
            await db.entityViews.updateEntityViewFilters(entityView.id, viewFilters);
            await db.entityViews.updateEntityViewSort(entityView.id, viewSort);
          })
        );
      }
  ...
  if (createdEntities.length > 0 && template.relationships) {
    await Promise.all(
      template.relationships.map(async (relationship) => {
        const fromEntity = await db.entities.getEntityByName({ tenantId: null, name: relationship.parent });
        const toEntity = await db.entities.getEntityByName({ tenantId: null, name: relationship.child });
        if (!fromEntity) {
          throw new Error(`Parent entity not found: ${relationship.parent}`);
        }
        if (!toEntity) {
          throw new Error(`Child entity not found: ${relationship.child}`);
        }
        return await db.entityRelationships.createEntityRelationship({

...
export async function seedRolesAndPermissions(adminEmail?: string): Promise<void> {
  ...
  await db.permissions.createPermissions(defaultPermissions);
  ...
  await Promise.all(
    entities.map(async (entity) => {
      return getEntityPermissions(entity).map(async (permission) => {
        ...
        return await db.permissions.createPermissions([entityPermission], appPermissions.length + 1);
      });
    })
  );

  const users = (await db.users.adminGetAllUsers()).items;
  await Promise.all(
    users
      .filter((f) => f.admin)
      .map(async (user) => {
        adminRoles.map(async (adminRole) => {
          if (adminEmail && user.email === adminEmail) {
            return await db.userRoles.createUserRole(user.id, adminRole.id);
          }
        });
      })
  );
    ...
    tenants.map(async (tenant) => {
      tenant.users.map(async (tenantUser) => {
        appRoles.map(async (appRole) => {
          return await db.userRoles.createUserRole(tenantUser.userId, appRole.id, tenant.id);
        });
      });
    })
  );

  const guestRole = await db.roles.getRoleByName(DefaultAdminRoles.Guest);
  if (guestRole) {
    const viewAndReadPermissions = (await db.permissions.getAllPermissions()).filter((f) => f.name.includes(".view") || f.name.includes(".read"));
    await Promise.all(
...
async function seedRoles(roles: CreateRoleDto[]) {
  const allRoles = await db.roles.getAllRoles();
  await Promise.all(
    roles.map(async (data, idx) => {
      ...
      const role = await db.roles.createRole({
        ...data,
        order: idx + 1,
        isDefault: true,
      });
  ...
  return await db.roles.getAllRoles();
}

...
import { ApiKeyDto } from "@/db/models/apiKeys/ApiKeysModel";
...
import { getOpenInvoices } from "@/utils/stripe.server";
import { getServerTranslations } from "@/i18n/server";
import { rateLimitByKey } from "./rateLimitService";

async function setApiError(request: Request, params: IServerComponentsProps, error: string, status: number, apiKeyLogId?: string) {
  if (apiKeyLogId) {
    await db.apiKeys.setApiKeyLogStatus(apiKeyLogId, {
      error,
      status,
      startTime: null,
    });
  } else {
    await db.apiKeys.createApiKeyLog(request, {
      apiKeyId: null,
      tenantId: null,
...
export async function validateApiKey(request: Request, params: IServerComponentsProps): Promise<ApiAccessValidation> {
  ...
  if (authorization && authorization.startsWith("Bearer ")) {
    ...
    const tenant = await db.tenants.getTenantByIdOrSlug(tenantId ?? "");
    return {
      userId,
...
export async function validateTenantApiKey(request: Request, params: IServerComponentsProps): Promise<ApiAccessValidation> {
  const { t } = await getServerTranslations();
  const apiKeyFromHeaders = request.headers.get("X-Api-Key") ?? "";
  const apiKey = await db.apiKeys.getApiKey(apiKeyFromHeaders);
  if (!apiKey) {
  ...
  const apiKeyLog = await db.apiKeys.createApiKeyLog(request, {
    apiKeyId: apiKey.id,
    tenantId: apiKey.tenantId,
    endpoint: new URL(request.url).pathname,
  });
  ...
  db.tenantIpAddress.createUniqueTenantIpAddress({
  ...
  if (usage && !usage.enabled) {
    await NotificationService.sendToRoles({
      ...
      notification: {
        ...
        action: {
          url: getBaseURL() + "/app/" + apiKey.tenant.slug + "/settings/api/keys",
        },
      },
    });
...
export async function getEntityApiKeyFromRequest(request: Request, params: Params) {
  ...
  const entitySlug = Array.isArray(params.entity) ? params.entity[0] : params.entity!;
  const entity = await db.entities.getSimpleEntityBySlug(entitySlug);
  if (!tenantApiKey) {

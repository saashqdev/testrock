...
import { get } from "./RowsApi";
import { EntityWithDetailsDto, EntityWithDetailsAndRelationshipsDto } from "@/db/models/entityBuilder/EntitiesModel";
import { db } from "@/db";
...
export async function getRelationship(
  ...
  const relationship = await time(db.rowRelationships.getRowRelationship(id), "RowRelationshipsApi.getRelationship");
  if (!relationship) {
    return null;
  }
  const parent = await get(relationship.parentId, {
    entity: {
  ...
  const child = await get(relationship.childId, {
    entity: {
...
export async function createRelationship({
  ...
  return await time(
    db.rowRelationships.createRowRelationship({
...
export async function deleteRelationship({
  ...
  const relationship = await time(
    db.entityRelationships.getEntitiesRelationship({
  ...
  return await time(
    db.rowRelationships.deleteRowRelationship({
      parentId: parent.id,
      childId: child.id,
    }),
...
async function getParentChildEntities({
  ...
  let parentEntity: EntityWithDetailsAndRelationshipsDto | null = null;
  let childEntity: EntityWithDetailsAndRelationshipsDto | null = null;
  if ("entityName" in parent) {
    parentEntity = allEntities
      ? allEntities.find((e) => e.name === parent.entityName) || null
      : await db.entities.getEntityByName({ tenantId: undefined, name: parent.entityName });
  } else {
    parentEntity = allEntities
      ? allEntities.find((e) => e.id === parent.entityId) || null
      : await db.entities.getEntityById({ tenantId: undefined, id: parent.entityId });
  }
  if ("entityName" in child) {
    childEntity = allEntities
      ? allEntities.find((e) => e.name === child.entityName) || null
      : await db.entities.getEntityByName({ tenantId: undefined, name: child.entityName });
  } else {
    childEntity = allEntities
      ? allEntities.find((e) => e.id === child.entityId) || null
      : await db.entities.getEntityById({ tenantId: undefined, id: child.entityId });
  }
  if (!parentEntity) {
...
export async function deleteRelationshipById(
  ...
  const relationship = await db.rowRelationships.getRowRelationship(id);
  if (!relationship) {

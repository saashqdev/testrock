...
export async function share(row: Row, data: { type: "tenant" | "user" | "role" | "group" | "public"; id?: string; access: RowAccess }) {
  ...
  const permissions = await db.rowPermissions.getRowPermissions(row.id);
  ...
  const create = await db.rowPermissions.createRowPermission({
    rowId: row.id,
    ...permission,
  });
...
export async function setAccess(id: string, access: RowAccess) {
  return await db.rowPermissions.updateRowPermission(id, {
    access,
  });
}

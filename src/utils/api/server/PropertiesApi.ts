...
export async function create({
  ...
  const property = await db.properties.createProperty({
    entityId,
    name,
    title,
    type,
  ...
  if (options !== undefined) {
    await db.properties.updatePropertyOptions(property.id, options);
  }
  if (attributes !== undefined) {
    await db.properties.updatePropertyAttributes(property.id, attributes);
  }
  return property;
}
export async function duplicate({ entity, propertyId }: { entity: EntityWithDetailsDto; propertyId: string }) {

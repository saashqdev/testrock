...
interface Props {
  items: TenantUserWithUserDto[];
}
export default function MembersListAndTable({ items }: Props) {
  const { t } = useTranslation();

  function getUserRole(item: TenantUserWithUserDto) {
          ...
          },
          {
            ...
            value: (item) => (
              <div className="flex items-center space-x-2">
                <Link href={"edit/" + item.id} className="text-theme-600 hover:text-theme-900 flex items-center space-x-2">

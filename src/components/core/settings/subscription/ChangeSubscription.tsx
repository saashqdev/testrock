...
interface Props {
  current: TenantSubscriptionWithDetailsDto | null;
  ...
  return (
    <div>
                ...
                .map((plan, index) => {
                  return (
                    <div key={index}>
                      <section
                        className={clsx(
                          "shadow-2xs relative flex w-full flex-col rounded-md p-6",
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <div className="shrink-0 truncate">
                            ...
                            {(() => {
                              if (getPrice(plan)?.billingPeriod === SubscriptionBillingPeriod.MONTHLY) {
                                return <span className="lowercase text-muted-foreground">/{t("pricing.MONTHLYShort")}</span>;
                              } else if (getPrice(plan)?.billingPeriod === SubscriptionBillingPeriod.YEARLY) {
                                return <span className="lowercase text-muted-foreground">/{t("pricing.YEARLYShort")}</span>;
                              } else if (getPrice(plan)?.billingPeriod === SubscriptionBillingPeriod.ONCE) {
                                return <span className="lowercase text-muted-foreground">/{t("pricing.once")}</span>;
                              }
                            })()}
                          </div>
                          ...
                          <div className="shrink-0 space-y-2 pt-4">
                            <button
                              ...
                              className={clsx(
                                "focus:outline-hidden inline-flex w-full items-center justify-center rounded-lg border px-4 py-2 text-sm transition-colors focus:ring-2 focus:ring-offset-2",
                                loading || isCurrent(plan) || !getPrice(plan)?.stripeId || !canSubscribe
                                  ? "cursor-not-allowed opacity-80"
                                  : "border-slate-800 hover:bg-secondary/90"
      ...
      <Modal className="sm:max-w-xs" open={showQuantityModal} setOpen={setShowQuantityModal}>
        <InputNumber name="quantity" title={t("pricing.seats")} value={quantity} onChange={(e) => setQuantity(Number(e))} required min={1} />
        {selectedPlan && (

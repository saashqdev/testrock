...
export default function UsageBasedPricesUnit({ item, onUpdate, disabled }: Props) {
  ...
  const [allTiers, setAllTiers] = useState<{ id: string; from: number; to?: number }[]>(item.tiers.map((t, idx) => ({ ...t, id: `tier-${idx}` })));
  ...
  useEffect(() => {
    ...
    commonHeaders = [
      ...
      },
      {
        ...
        value: (e) => (e.to ? e.to : "∞"),
        formattedValue: (e) => (
          <div className="flex justify-center font-medium text-muted-foreground">{e.to ? <span>{NumberUtils.numberFormat(e.to)}</span> : <span>∞</span>}</div>
        ),
        className: "w-24",
      },
    ];

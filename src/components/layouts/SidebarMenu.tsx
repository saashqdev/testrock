...
export default function SidebarMenu({ layout, onSelected, menuItems }: Props) {
  ...
  function checkFeatureFlags(item: SideBarItem) {
    return !item.featureFlag || !rootData?.featureFlags || rootData.featureFlags.includes(item.featureFlag);
  }
    ...
    <div>
      ...
      <div className="space-y-1 divide-y-2 divide-slate-800 sm:hidden">
        {getMenu().map((group, index) => {
          return (
            <div key={group.title || index} className="mt-2">
              ...
              {group.items.map((menuItem, itemIndex) => {
                return (
                  <div key={menuItem.path || `${menuItem.title}-${itemIndex}`}>
                    {(() => {
                            ...
                            {menuItemIsExpanded(menuItem.path) && (
                              <div className="mt-1">
                                {menuItem.items.map((subItem, subIndex) => {
                                  return (
                                    <Fragment key={subItem.path || `${subItem.title}-${subIndex}`}>
                                      {menuItem.path.includes("://") ? (
                                        <a
                                          target="_blank"
                                          href={menuItem.path}
      ...
      <div className="hidden space-y-1 divide-y-2 divide-slate-800 sm:block">
        {getMenu().map((group, index) => {
          return (
            <div key={group.title || index} className="select-none">
              <div className="mt-2">

    ...
    <Listbox value={selected} onChange={setSelected} disabled={disabled || readOnly}>
      {({ open }) => (
        <div className={clsx(className, "text-slate-50")}>
          {withLabel && title && (
            <Listbox.Label htmlFor={name} className="mb-1 flex justify-between space-x-2 text-xs font-medium text-muted-foreground">
          ...
          <div className="relative">
            <Listbox.Button
              ...
              ref={button}
              className={clsx(
                "focus:ring-accent-700 shadow-2xs focus:outline-hidden relative w-full cursor-default rounded-sm border border-slate-800 bg-slate-800 py-2 pl-3 pr-10 text-left text-slate-50 hover:bg-slate-700 focus:border-border focus:ring-1 sm:text-sm",
            ...
            <Transition show={open} as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
              <Listbox.Options
                ...
                className={clsx(
                  "focus:outline-hidden absolute z-10 mt-1 max-h-72 w-full overflow-auto rounded-sm bg-slate-800 py-1 text-base text-white shadow-lg ring-1 ring-black ring-opacity-5 sm:text-sm"
                )}
              >
                  ...
                  <div className="flex rounded-sm p-2">
                    <div className="relative flex grow items-stretch focus-within:z-10">
                      ...
                      <input
                        ...
                        className="focus:border-accent-300 focus:outline-hidden block w-full rounded-none rounded-l-md border border-border bg-background px-3 py-2 pl-10 text-sm focus:ring-slate-300 sm:text-sm"
                      />
                    </div>
                    {onNew && (
                      <button
                        ...
                        type="button"
                        onClick={onNew}
                        className="focus:outline-hidden relative -ml-px inline-flex items-center space-x-2 rounded-r-md border border-border bg-secondary px-2 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 focus:border-border focus:ring-1 focus:ring-ring"
                      >
                  ...
                  <Fragment key={idx}>
                    {group.name && (
                      ...
                      >
                        {({ selected, active }) => (
                          <Fragment>
                            <span className={clsx("text-xs font-medium text-muted-foreground", selected ? "font-semibold" : "font-normal")}>{group.name}</span>
                          </Fragment>
                        )}
                      </Listbox.Option>
                    )}

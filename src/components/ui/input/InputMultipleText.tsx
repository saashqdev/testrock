...
import { SeparatorFormatType } from "@/lib/shared/SeparatorUtils";
    ...
    <div className={clsx(className, !darkMode && "")}>
      ...
      <div className={clsx("relative flex w-full rounded-md")}>
        ...
        <div className="flex w-full flex-wrap items-center">
          {value.map((tag, index) => (
            <div key={index} className="m-0.5 flex items-center rounded border border-border bg-secondary px-2 py-2 text-sm">
              <span>{tag}</span>
              {!disabled && !readOnly && (
                <button
                  type="button"
                  disabled={disabled}
                  className="ml-2 shrink-0 text-muted-foreground focus:text-foreground"

          ...
          <RadioGroup.Option
            ...
            className={({ checked, active }) =>
              clsx(
                checked ? "ring-ring" : "",
                active ? "" : "",
                "shadow-2xs focus:outline-hidden relative flex rounded-lg border p-3",
                disabled || item.disabled ? "cursor-not-allowed text-muted-foreground opacity-60" : "cursor-pointer"
              )
            }
          >
            {({ checked, active }) => (
              <>
                ...
                <CheckFilledCircleIcon className={clsx("shrink-0", !checked ? "invisible hidden" : "flex", "h-5 w-5 text-primary")} aria-hidden="true" />
                <span
                  className={clsx(
                    checked || active ? "border-transparent ring-2 ring-ring" : "",

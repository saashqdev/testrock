...
//         <div
//           className={clsx(
//             "mt-6 hidden text-sm font-medium text-muted-foreground sm:grid",
...
export default function ProgressBar({ steps }: { steps: StepDto[] }) {
  ...
  return (
    <nav aria-label="Progress">
      <ol className="divide-gray-300 rounded-t-md border border-border bg-background sm:divide-y lg:flex lg:divide-y-0">
        {steps.map((step, stepIdx) => (
          <li key={step.name} className="relative md:flex md:flex-1">
            {step.status === "completed" ? (
              <div className="group hidden w-full items-center sm:flex">
                <span className="flex items-center px-6 py-4 text-sm font-medium">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-theme-600">
                    <CheckIcon className="h-6 w-6 text-white" aria-hidden="true" />
                  </span>
                  <span className="ml-3 truncate text-sm font-medium text-foreground">{step.title}</span>
                </span>
              </div>
            ) : step.status === "current" ? (
              <div className="flex items-center px-6 py-4 text-sm font-medium" aria-current="step">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 border-theme-600">
              ...
              <div className="group hidden items-center sm:flex">
                <span className="flex items-center px-6 py-4 text-sm font-medium">
                  ...
                  <span className="ml-3 truncate text-sm font-medium text-muted-foreground">{step.title}</span>
                </span>
              </div>
            )}

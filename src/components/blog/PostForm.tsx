...
interface Props {
  item?: BlogPostWithDetailsDto | null;
  ...
  async function yesRemove() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        errorModal.current?.show(t("shared.error"), errorData.error || "Failed to delete post");
      }
    } catch (error) {
      errorModal.current?.show(t("shared.error"), "An error occurred while deleting the post");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json().catch(() => ({ error: "An error occurred" }));
        errorModal.current?.show(t("shared.error"), errorData.error || "Failed to save post");
      }
    } catch (error) {
      errorModal.current?.show(t("shared.error"), "An error occurred while saving the post");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="grid gap-3 lg:grid-cols-12">
        ...
        <div className="space-y-3 lg:col-span-4">
          ...
          <div className="flex justify-between space-x-2">
            <div>
              {item && (
                <ButtonSecondary destructive={true} disabled={loading || isPending || !canDelete} onClick={remove}>
            ...
            <div className="flex items-center space-x-2">
              <ButtonSecondary onClick={() => router.push(UrlUtils.getModulePath(params, "blog"))} disabled={loading || isPending}>
                <div>{t("shared.cancel")}</div>
              </ButtonSecondary>
              {/* <ButtonSecondary onClick={preview} disabled={loading || isPending}>
            {t("shared.preview")}
          </ButtonSecondary> */}
              <LoadingButton type="submit" disabled={loading || isPending || !canUpdate}>
                {t("shared.save")}
              </LoadingButton>
            </div>
          </div>

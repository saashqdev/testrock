...
import { PropertyWithDetailsDto, EntityDto } from "@/db/models/entityBuilder/EntitiesModel";
        ...
        <div className="mt-4">
          <div className="space-y-3 px-4">
            <div className="w-full">
              <label htmlFor="type" className="block text-xs font-medium text-foreground/80">
            ...
            {type === PropertyType.TEXT && (
              <div>
                <label htmlFor="subtype" className="block text-xs font-medium text-foreground/80">
            ...
            {type === PropertyType.FORMULA && (
              <div>
                <div className="flex justify-between space-x-2">
                  ...
                  {item?.formulaId ? (
                    <Link href={`/admin/entities/formulas/${item?.formulaId}`} className="text-xs text-blue-500 hover:underline">
                      Go to formula
                    </Link>
                  ) : (
                    <Link href={`/admin/entities/formulas`} className="text-xs text-blue-500 hover:underline">
                      View formulas
                    </Link>
                  )}
                </div>
            ...
            <InputText
              ...
              hint={<span className="font-normal italic text-muted-foreground">Camel case</span>}
            />

            <div className="w-full rounded-md border border-dotted border-border px-2 py-0">
              ...
              <div className="w-full">
                ...
                <div className="mt-1">
                  <InputText
                    ...
                    value={formula}
                    onChange={(e) => setFormula(e.toString())}
                    disabled={!titleEnabled}
                    required
                  />
                </div>
            ...
            {[PropertyType.SELECT, PropertyType.MULTI_SELECT].includes(type) && (
              <div className="w-full">
                <label className="block text-sm font-medium text-foreground/80">Options</label>
                <div className="shadow-2xs mt-1 flex rounded-md">
                  <div className="relative flex grow items-stretch focus-within:z-10">
                    <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                      <ViewListIcon className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
                    </div>
                    {options.map((option) => {
                    ...
                    <input
                      disabled
                      className="block w-full rounded-none rounded-l-md border border-border bg-secondary/90 pl-10 text-foreground focus:border-border focus:ring-ring sm:text-sm"
                  ...
                  <button
                    ...
                    className="focus:outline-hidden relative -ml-px inline-flex items-center space-x-2 rounded-r-md border border-border bg-secondary px-4 py-1.5 text-sm font-medium text-foreground/80 hover:bg-secondary/90 focus:border-border focus:ring-1 focus:ring-ring"
                  >
                    <PencilAltIcon className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
                    <span>Set</span>
                  </button>
                </div>
              </div>
          ...
          <div className={clsx("my-2 space-y-3 border border-dashed border-border bg-secondary px-4 py-3", showAdvancedOptions ? "" : "hidden")}>
            <div className="w-full">
              <InputNumber name="order" title={t("models.property.order")} value={order} onChange={setOrder} />
            </div>

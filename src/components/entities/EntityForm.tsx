...
import VisibilityHelper from "@/lib/helpers/VisibilityHelper";
...
export default function EntityForm({ item, onSubmit }: Props) {
  ...
  const [order, setOrder] = useState<number | undefined>(
    item?.order ?? (adminData?.entities && adminData.entities.length > 0 ? Math.max(...adminData.entities.map((o) => o.order)) : 0) + 1
  );
  ...
  return (
    <FormGroup id={item?.id} onCancel={() => router.push("/admin/entities")} editing={true} canDelete={false} onSubmit={onSubmit}>
      ...
      <InputGroup title="Behavior">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-12">
          <InputSelect
            ...
            title="On created"
            value={onCreated}
            onChange={(e) => setOnCreated(e?.toString() ?? "")}
            options={[
          ...
          <InputSelect
            ...
            title="On edit"
            value={onEdit}
            onChange={(e) => setOnEdit(e?.toString() ?? "")}
            options={[
      ...
      <InputGroup title="Settings">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-12">
          <div className="col-span-12 divide-y divide-border">

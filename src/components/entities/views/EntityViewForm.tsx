...
import { ViewFilterCondition } from "@/lib/enums/entities/ViewFilterCondition";
...
import OrderHelper from "@/lib/helpers/OrderHelper";
...
import { defaultDisplayProperties } from "@/lib/helpers/PropertyHelper";

interface Props {
  entity: EntityWithDetailsDto | undefined;
  item?: EntityViewsWithDetailsDto | null;
  canDelete?: boolean;
  onClose?: () => void;
  actionNames?: {
    create: string;
...
export default function EntityViewForm({
  ...
  showViewType,
  onSubmit,
}: Props & { entity: EntityWithDetailsDto }) {
  ...
  const [gridGap, setGridGap] = useState<string | number | undefined>(item?.gridGap ?? "sm");
  ...
  return (
    <FormGroup id={item?.id} onCancel={onClose} editing={true} canDelete={canDelete} actionNames={actionNames} className="text-foreground" onSubmit={onSubmit}>
      ...
      <InputGroup title="View">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-12">
              ...
              return {
                ...
                renderName: (
                  <div className="flex items-center space-x-2">
                    <EntityViewLayoutBadge layout={f.value} className="h-3 w-3 text-muted-foreground" />
                    <div>{f.name}</div>
                  </div>
                ),
              };
          ...
          {!isSystem && (
            <Fragment>
              <InputNumber className="sm:col-span-3" name="pageSize" title={"Page size"} value={pageSize} onChange={setPageSize} min={1} max={500} required />
              <InputNumber
                ...
                required
              />
              <InputCheckboxInline className="sm:col-span-12" name="isDefault" title="Is default" value={isDefault} onChange={setIsDefault} />
            </Fragment>
          )}
        </div>
      </InputGroup>
      ...
      {layout === "board" && (
        <InputGroup title="Board">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-12">
            <>
              <InputSelect className="sm:col-span-6" name="groupBy" title="Group by" value={groupBy} onChange={setGroupBy} options={groupByOptions} />
              <InputSelect
                className="sm:col-span-6"
                name="groupByPropertyId"
                title="Property"
                value={groupByPropertyId}
                onChange={setGroupByPropertyId}
      ...
      {layout === "grid" && (
        <InputGroup title="Grid">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-7">
            <InputNumber name="gridColumns" title="Columns" value={gridColumns} onChange={setGridColumns} min={0} max={12} required />
            <InputNumber name="gridColumnsSm" title="sm" value={gridColumnsSm} onChange={setGridColumnsSm} min={0} max={12} required />
            <InputNumber name="gridColumnsMd" title="md" value={gridColumnsMd} onChange={setGridColumnsMd} min={0} max={12} required />
            <InputNumber name="gridColumnsLg" title="lg" value={gridColumnsLg} onChange={setGridColumnsLg} min={0} max={12} required />
            <InputNumber name="gridColumnsXl" title="xl" value={gridColumnsXl} onChange={setGridColumnsXl} min={0} max={12} required />
            <InputNumber name="gridColumns2xl" title="2xl" value={gridColumns2xl} onChange={setGridColumns2xl} min={0} max={12} required />
        ...
        <div className="divide-y divide-border">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              <div className="h-64 space-y-1 overflow-y-scroll rounded-md border-2 border-dashed border-border p-2">
                    ...
                    <InputCheckboxInline
                      ...
                      title={
                        <div className="flex items-baseline space-x-1 truncate text-sm">
                          <div className={clsx("truncate", item.name?.includes(".") ? "text-muted-foreground" : "font-medium text-foreground")}>
                            {item.title ? t(item.title) : item.name}
                          </div>
                          <div
                            className={clsx(
                              "truncate text-xs font-normal",
                              item.name?.includes(".") ? "italic text-muted-foreground" : "font-bold text-foreground"
                            )}
                          >
                            ({item?.name})
                          </div>
            ...
            <div className="space-y-1">
              <div className="flex items-baseline space-x-1">
                ...
                <button type="button" className="text-xs text-muted-foreground" onClick={() => setProperties([])} disabled={properties.length === 0}>
                  ({t("shared.clear")})
                </button>
              </div>
              <div className="h-64 space-y-1 overflow-y-scroll rounded-md border-2 border-dashed border-border p-2">
                  ...
                  .map((item, idx) => (
                    <div key={idx} className="flex items-baseline space-x-1 text-sm">
                      ...
                      <div className={clsx("truncate", item.name?.includes(".") ? "text-muted-foreground" : "font-medium text-foreground")}>
                        ...
                        <span className={clsx("text-xs font-normal", item.name?.includes(".") ? "italic text-muted-foreground" : "font-bold text-foreground")}>
                          ({item?.name})
                        </span>
                      </div>
                    </div>
      ...
      {!isSystem && (
        <Fragment>
          <InputGroup title="Filters">
                ...
                <CollapsibleRow
                  ...
                  value={
                    <div className="flex items-center space-x-1 text-sm">
                      ...
                      <div className="font-light text-muted-foreground">{item.condition}</div>
            ...
            >
              <span className="block text-xs font-normal text-muted-foreground">{filters.length === 0 ? "No filters" : "Add filter"}</span>
            </button>
          </InputGroup>

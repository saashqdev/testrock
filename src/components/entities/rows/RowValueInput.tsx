...
import { getSeparator } from "@/lib/shared/SeparatorUtils";
...
interface Props {
  selected: PropertyWithDetailsDto | undefined;
  ...
  className?: string;
  autoFocus?: boolean;
  promptFlows?: { rowId: string | undefined; prompts: PromptFlowWithDetailsDto[] } | undefined;
}
    ...
    <>
      {selected?.type === PropertyType.TEXT ? (
          ...
          {["monaco", "wysiwyg"].includes(PropertyAttributeHelper.getPropertyAttributeValue_String(selected, PropertyAttributeName.Editor) ?? "") &&
            readOnly && (
              <div className="overflow-auto">
                ...
                <div className="prose h-auto rounded-md border border-border bg-secondary p-3">
                  <div
        ...
        <InputNumber
          ...
          required={selected.isRequired}
          value={numberValue}
          onChange={(e) => (onChange ? onChange(Number(e)) : undefined)}
          disabled={readOnly}
          className={className}
          readOnly={readOnly}
...
function InputFormulaValue(props: { property: PropertyWithDetailsDto; textValue?: string; numberValue?: number; dateValue?: Date; booleanValue?: boolean }) {
  return (
    <>

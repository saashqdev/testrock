    ...
    <div className="mx-auto mb-12 max-w-5xl space-y-5 px-4 py-4 sm:px-6 lg:px-8 xl:max-w-7xl">
      ...
      <dl className="grid gap-2 sm:grid-cols-3">
        <div className="shadow-xs overflow-hidden rounded-lg border border-border bg-card px-4 py-3">
          ...
          <dd className="mt-1 truncate text-2xl font-semibold text-foreground">{NumberUtils.intFormat(data.subscribers?.totalCount ?? 0)}</dd>
        </div>
        <div className="shadow-xs overflow-hidden rounded-lg border border-border bg-card px-4 py-3">
          <dt className="truncate text-xs font-medium uppercase text-muted-foreground">
            <div>Messages</div>
          </dt>
          <dd className="mt-1 truncate text-2xl font-semibold text-foreground">{NumberUtils.intFormat(data.messages?.totalCount ?? 0)}</dd>
        </div>
        <div className="shadow-xs overflow-hidden rounded-lg border border-border bg-card px-4 py-3">

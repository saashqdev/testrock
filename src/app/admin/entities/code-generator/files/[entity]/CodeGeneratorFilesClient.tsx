...
export default function CodeGeneratorFilesClient({ entity }: { entity: EntityWithDetailsDto }) {
  ...
  useEffect(() => {
    ...
    if (!file && files.length > 0) {
      ...
      params.set("file", files[0].file);
      router.push(`?${params.toString()}`);
    }
  }, [files, searchParams, router, mounted]);
            ...
            <div className="flex flex-col space-y-2 md:flex-row md:space-x-2 md:space-y-0">
              <div className="md:w-5/12">
                ...
                <InputSelect
                  ...
                  onChange={(i) => {
                    const params = new URLSearchParams(searchParams.toString());
                    params.set("file", files[i as number].file);
                    router.push(`?${params.toString()}`);
                  }}
                />
              </div>

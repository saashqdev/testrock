...
export default function DangerClient({ entity, count }: DangerClientProps) {
  ...
  function onDelete(action: string) {
    ...
    if (action === "delete") {
      title = `Delete ${entity.title} entity`;
      button = `Delete ${entity.title} entity`;
    } else {
      title = `Delete ${entity.title} rows (${count})`;
      button = `Delete ${entity.title} rows (${count})`;
    }

...
import type { ContactFormSettings } from "@/modules/crm/services/CrmService";
import type { PageLoaderData } from "@/modules/pageBlocks/dtos/PageBlockData";
...
function ContactForm({ isSubmitting }: { isSubmitting?: boolean }) {
  const { t } = useTranslation();
  ...
  return (
    <div className="mt-9 grid grid-cols-1 gap-x-1 gap-y-4 sm:grid-cols-2 sm:gap-x-4">
      ...
      <div className="text-right sm:col-span-2">
        ...
        >
          {isSubmitting ? t("shared.loading") || "Sending..." : t("front.contact.send")}
        </ButtonPrimary>
      </div>
    </div>
  );

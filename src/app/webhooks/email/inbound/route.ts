...
export async function POST(request: Request) {
  try {
    ...
    const tenantWithInboundAddresses = await db.tenantInboundAddresses.getTenantInboundAddress(fromInboundAddress.map((f) => f.split("@")[0]));
    ...
    const createdEmail = await db.emails.createEmail({
    ...
    const emailWithAttachments = await db.emails.getEmail(createdEmail.id, tenantWithInboundAddress?.tenantId ?? null);
          ...
          emailWithAttachments?.attachments.map(async (attachment) => {
            ...
            try {
              ...
              if (createdFile.publicUrl) {
                return await db.emailAttachments.updateEmailAttachmentFileProvider(attachment.id, {
                  content: "",

...
async function callEventEndpoint({ request, event, endpoint, body }: { request: Request; event: Event; endpoint: string; body: string }) {
  const webhookAttempt = await db.eventWebhookAttempts.createEventWebhookAttempt({ eventId: event.id, endpoint });
  try {
    await fetch(getBaseURL() + `/api/events/webhooks/attempts/${webhookAttempt.id}`, {
      method: "POST",
      headers: {
    ...
    await db.eventWebhookAttempts.updateEventWebhookAttempt(webhookAttempt.id, {
      startedAt: new Date(),
      finishedAt: new Date(),
      success: false,
      status: 500,

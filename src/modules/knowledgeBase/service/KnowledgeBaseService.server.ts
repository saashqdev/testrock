...
async function getAll({ enabled, request }: { enabled?: boolean; request: Request }): Promise<KnowledgeBaseDto[]> {
  const items = await db.knowledgeBase.getAllKnowledgeBases({ enabled });
...
async function get({ slug, enabled, request }: { slug: string; enabled?: boolean; request: Request }): Promise<KnowledgeBaseDto> {
  const item = await cachified({
    key: `kb:${slug}`,
    ttl: 60 * 60 * 24,
    getFreshValue: () => db.knowledgeBase.getKnowledgeBaseBySlug(slug),
  });
  if (!item) {
...
function kbToDto(kb: KnowledgeBaseWithDetailsDto, request: Request) {
...
async function getCategories({
  ...
  items.forEach((category) => {
    category.articles = category.articles.filter((f: any) => f.publishedAt);
  });
...
async function getArticle({ kb, slug, params, request }: { kb: KnowledgeBaseDto; slug: string; params: { lang?: string }; request: Request }): Promise<{
  ...
  const item = await db.kbArticles.getKbArticleBySlug({
  ...
  const category = await db.kbCategories.getKbCategoryById(item.categoryId);
  if (!category) {
    return null;
  }
  return {
...
async function getFeaturedArticles({ kb, params, request }: { kb: KnowledgeBaseDto; params: { lang?: string }; request: Request }) {
  const items = await db.kbArticles.getFeaturedKnowledgeBaseArticles({
    knowledgeBaseId: kb.id,
    language: params.lang || kb.defaultLanguage,
  });
  return items.map((f) => articleToDto({ kb, article: f, relatedArticles: f.relatedArticles, params, request }));
...
function articleToDto({
  ...
  kb: KnowledgeBaseDto;
  article: KnowledgeBaseArticleWithDetailsDto;
...
async function getCategory({
  ...
  const item = await db.kbCategories.getKbCategoryBySlug({
    knowledgeBaseId: kb.id,
    slug: category,
    language,
  });
  ...
  item.articles = item.articles.filter((f: any) => f.publishedAt);
  return categoryToDto({ kb, category: item, params, request });
}

async function del(item: KnowledgeBaseDto) {
  const articlesCount = await prisma.knowledgeBaseArticle.count({
    where: { knowledgeBaseId: item.id },
  });
  const categoriesCount = await prisma.knowledgeBaseCategory.count({
    where: { knowledgeBaseId: item.id },
  });
  if (articlesCount > 0) {
...
async function duplicateCategory({ kb, language, categoryId }: { kb: KnowledgeBaseDto; language: string; categoryId: string }) {
  ...
  const item = await db.kbCategories.createKnowledgeBaseCategory({
    knowledgeBaseId: kb.id,
    slug,
    order: maxOrder + 1,
...
async function duplicateArticle({ kb, language, articleId }: { kb: KnowledgeBaseDto; language: string; articleId: string }) {
  ...
  const item = await db.kbArticles.createKnowledgeBaseArticle({
    knowledgeBaseId: kb.id,
...
async function search({
  ...
  const articles = await db.kbArticles.findKnowledgeBaseArticles({
    knowledgeBaseSlug: kb.slug,
    language: params.lang || kb.defaultLanguage,
    query,
  });
...
async function newArticle({
  ...
  const created = await db.kbArticles.createKnowledgeBaseArticle({
    knowledgeBaseId: kb.id,
...
async function contentAsText(text: string) {

...
export default function KbArticleContent({
  ...
  return (
    <div>
      <div className="flex flex-col space-y-6">
        <div className="flex items-center space-x-8">
          <div className="flex w-full flex-col">
            ...
            <p className="mt-2 font-light text-muted-foreground">{item.description}</p>
            <div className="flex justify-between space-x-2">
                ...
                <div>
                  {item.createdByUser && (
                    <dl className="whitespace-no-wrap text-sm font-medium leading-5">
                      ...
                      <dd className="">
                        <span className="font-normal text-muted-foreground">{t("shared.by")}</span> {item.createdByUser.firstName} {item.createdByUser.lastName}
                      </dd>
                    </dl>
                  )}
              ...
              {actions && (
                <div className="flex items-center space-x-2">
                  <button type="button" onClick={actions.onThumbsUp} className="flex items-center space-x-2 text-muted-foreground hover:text-foreground">
                    {userState?.hasThumbsUp ? <ThumbsUpIconFilled className="h-4 w-4" /> : <ThumbsUpIcon className="h-4 w-4" />}
                  </button>
                  <button type="button" onClick={actions.onThumbsDown} className="flex items-center space-x-2 text-muted-foreground hover:text-foreground">
      ...
      <div className="border-b border-border pt-6"></div>
      ...
      <div className="grid gap-8 sm:grid-cols-12">
        ...
        {item.relatedArticles.length > 0 && (
          <div className="space-y-3 py-5 sm:col-span-3">
            <div className="text-sm font-medium text-muted-foreground">Related Articles</div>
            <div className="space-y-2">
              {item.relatedArticles.map((x) => (
                <Link href={x.href} key={x.href} className="flex items-center space-x-2 hover:underline">

...
export default function KnowledgeBaseForm({
  ...
  return (
    <div>
      <form onSubmit={handleFormSubmit} method="post" className="inline-block w-full overflow-hidden p-1 text-left align-bottom sm:align-middle">
        ...
        <div className="space-y-2">
          ...
          <InputSelect
            ...
            onChange={(e) => setDefaultLanguage(e?.toString() || "en")}
          ...
          <InputSelect
            ...
            title={"Layout"}
            value={layout}
            onChange={(e) => setLayout((e?.toString() as any) || "list")}
            options={[
              {
                value: "list",
          ...
          <InputText
            ...
            button={
              <div className="absolute inset-y-0 right-0 flex py-0.5 pr-0.5">
                <kbd
                  className={clsx(
                    "inline-flex w-auto items-center justify-center rounded border border-border px-1 text-center font-sans text-xs font-medium text-muted-foreground",
                    ColorBackgroundUtils.getBg700(color)
                  )}
                >
                  {logo === "light" ? (
                    <Image className="h-7 w-auto" src={LogoLight} alt="Logo" />
                  ) : logo === "dark" ? (
                    <Image className="h-7 w-auto" src={LogoDark} alt="Logo" />
                  ) : logo.startsWith("http") ? (
                    <Image className="h-7 w-auto" src={logo} alt="Logo" />
                  ) : (

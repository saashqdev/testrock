        ...
        <div className="mt-4 space-y-2" key={idx}>
          <div className="flex space-x-2">
            ...
            <InputNumber
              ...
              onChange={(e) => setTemplate({ ...template, temperature: Number(e) })}
              required
              min={0.1}
              max={1.0}
              step={"0.1"}
            ...
            <div className="grid grid-cols-12 gap-3">
              ...
              <div className="col-span-12 h-[calc(100vh-240px)] space-y-2 overflow-auto md:col-span-4 lg:col-span-3">
                <div>
                  ...
                  <div className="flex h-full w-full flex-wrap items-center rounded-md border border-border bg-gray-50">
                    {PromptFlowUtils.getUsedVariables(template.template ?? "", metadata.entities).length === 0 ? (
                      <div className="m-1 flex items-center rounded border border-border bg-secondary/90 px-2 py-2 text-sm">
                        <span>None</span>
                      </div>
                    ) : (
                      <>
                        {PromptFlowUtils.getUsedVariables(template.template ?? "", metadata.entities).map((item, index) => (
                          <div key={index} className="m-1 flex select-all items-center rounded border border-border bg-secondary/90 px-2 py-2 text-sm">
                            <span>{item.name}</span>
                          </div>
                        ))}
                      </>

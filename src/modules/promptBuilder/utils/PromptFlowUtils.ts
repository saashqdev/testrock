...
function getUsedVariables(template: string, entities: EntityWithDetailsDto[]): PromptBuilderVariableDto[] {
...
function getPossibleVariables(entities: EntityWithDetailsDto[], upToIdx?: number): PromptBuilderVariableDto[] {
...
function getVariablesNeeded(flow: PromptFlowWithDetailsDto, allEntities: EntityWithDetailsDto[]) {
...
function getPromptFlowsOfType({ type, promptFlows, row }: { type: "list" | "edit"; promptFlows: PromptFlowWithDetailsDto[]; row?: RowDto }) {
  let items: PromptFlowWithDetailsDto[] = [];
  if (type === "list") {

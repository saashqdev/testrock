...
async function executeFlow({
  ...
  const data = TemplateApiHelper.getTemplateValue({
    ...
    t,
    session: {
      tenant: tenantId ? await db.tenants.getTenant(tenantId) : null,
      user: userId ? await db.users.getUser(userId) : null,
    },
    variables,
  ...
  const { id } = await db.promptExecutions.createPromptFlowExecution({
    flowId: flow.id,
    userId: userId ?? null,
    tenantId,
    status: "pending",
  ...
  await db.promptExecutions.updatePromptFlowExecution(execution.id, {
    status: "running",
    startedAt: new Date(),
  });
  ...
  const executionResult = await db.promptExecutions.getPromptFlowExecution(id);
...
async function executeTemplate({
  ...
  execution: PromptFlowExecutionWithResultsDto;
  templateResult: PromptTemplateResultWithTemplateDto;
  prompt: string;
  isDebugging?: boolean;
  userId?: string | null;
  model?: string;
  ...
  await db.promptExecutions.updatePromptTemplateResult(templateResult.id, {
    startedAt: new Date(),
    status: "running",
    prompt,
  });
...
async function runFromForm({
  ...
  const flow = await db.promptFlows.getPromptFlow(promptFlowId);
  if (!flow) {
  ...
  let row: RowWithDetailsDto | null = null;
  if (rowId) {
    row = await db.rows.getRowById(rowId);
  }
...
async function createDefault(promptTitle?: string) {
  ...
  const createdFlows = await Promise.all(
    toCreate.map(async ({ model, stream, title, description, actionTitle, executionType, promptFlowGroupId, inputEntityId, isPublic, templates }) => {
      await db.promptFlows.createPromptFlow({
        model,
        stream,
        title,
        description,

...
export default function MySubscriptionFeatures({ features, className = "", withCurrentPlan = false }: Props) {
            ...
            {features.map((item, idx) => {
              return (
                <div key={idx} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                  <dt className="truncate text-xs uppercase tracking-wide text-gray-500">{item.name}</dt>
                  <dd className="mt-1">

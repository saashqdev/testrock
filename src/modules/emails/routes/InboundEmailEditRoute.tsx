...
interface InboundEmailRouteProps {
  data: LoaderDataInboundEmailEdit;
  onDeleteAction: () => Promise<void>;
}

export default function InboundEmailRoute({ data, onDeleteAction }: InboundEmailRouteProps) {
    ...
    <div>
      ...
      >
        <div className="space-y-1 p-2">
          <div className="shadow-xs grid grid-cols-12 gap-3 rounded-md border border-border bg-background p-4">
            ...
            <div className="col-span-10">
              <div className="flex flex-col text-sm">
                ...
                <div className="select-all truncate text-sm text-muted-foreground">{data.item.fromEmail}</div>
              </div>
            </div>
            ...
            <div className="col-span-10">
              <div className="flex flex-col text-sm">
                ...
                <div className="select-all truncate text-sm text-muted-foreground">{data.item.toEmail}</div>
              </div>
            </div>
            ...
            <div className="col-span-10">
              <div className="flex items-baseline space-x-1 text-sm">
                ...
                <div className="text-xs text-muted-foreground">- {DateUtils.dateYMDHMS(data.item.date)}</div>
              </div>
            </div>

            <div className="col-span-2 text-end">
              <div className="text-xs text-muted-foreground">Read at: </div>
            </div>
            <div className="col-span-10">
              <div className="flex items-baseline space-x-1 text-sm">
                <div className="font-medium">{DateUtils.dateAgo(data.myRead.createdAt)}</div>
                <div className="text-xs text-muted-foreground">- {DateUtils.dateYMDHMS(data.myRead.createdAt)}</div>
              </div>
            </div>
                  ...
                  return (
                    <div key={item.id} className="truncate">
                      <div className="flex items-center space-x-2 truncate text-sm">
                        <div
                          className={clsx(
                            "w-10 shrink-0 truncate rounded-md border border-border bg-secondary p-0.5 text-center text-xs uppercase text-muted-foreground",
          ...
          <iframe
            className="shadow-xs h-full min-h-screen w-full rounded-md border border-border bg-background p-4"
            title={data.item.subject}

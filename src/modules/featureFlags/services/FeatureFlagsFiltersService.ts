...
async function matches({
  ...
  if (filter.type.startsWith("analytics.")) {
    uniqueVisitor = await prisma.analyticsUniqueVisitor.findUnique({
  ...
  let user: UserWithoutPasswordDto | null = null;
  if (filter.type.startsWith("user.")) {
    user = await db.users.getUser(userInfo.userId);
    if (!user) {
      return false;
    }
  }
  ...
  switch (type) {
    ...
    case "user.roles.notContains": {
      const userRoles = await db.userRoles.getUserRoles(user!.id);

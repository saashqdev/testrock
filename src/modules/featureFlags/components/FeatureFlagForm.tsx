      ...
      >
        <form action={formAction} className="inline-block w-full overflow-hidden p-1 text-left align-bottom sm:align-middle">
          ...
          <div className="space-y-2">
            ...
            <div>
              <div className="mb-1 flex items-center justify-between space-x-2 text-xs">
                <label className="font-medium text-muted-foreground">{t("featureFlags.filters")}</label>
                ...
                {filters.map((filter, idx) => {
                  return (
                    <button
                      ...
                      className="relative block w-full rounded-lg border-2 border-dashed border-border p-3 text-center hover:border-border focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                    >
          ...
          <div className="mt-5 flex justify-between space-x-2 sm:mt-6">
            <div>
              {onDelete && (
                <ButtonSecondary type="button" className="w-full" onClick={onDelete} destructive>
                  <div>{t("shared.delete")}</div>
                </ButtonSecondary>
              )}
            </div>
            <div className="flex space-x-2">
              <ButtonSecondary onClick={() => router.push("/admin/feature-flags/flags")}>{t("shared.cancel")}</ButtonSecondary>
              <LoadingButton actionName={getActionName()} type="submit">
                {t("shared.save")}
              </LoadingButton>
            </div>
          </div>

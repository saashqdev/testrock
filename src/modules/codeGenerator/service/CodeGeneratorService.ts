...
async function generate(options: CodeGeneratorOptions) {
  const moduleFiles = await getModuleFiles(options);
  await writeFiles(moduleFiles, options.moduleDirectory);

  if (options.routesDirectory) {
    const routeFiles = await getRouteFiles(options);
...
async function getModuleFiles(options: CodeGeneratorOptions): Promise<CodeGeneratorFileDto[]> {
...
async function getRouteFiles({ type, entity, moduleDirectory }: CodeGeneratorOptions) {
...
async function addTopComments(files: CodeGeneratorFileDto[]) {
  files.forEach((file) => {
...
async function moduleDirectoryOptions(entity: EntityWithDetailsDto) {
...
async function routeDirectoryOptions(entity: EntityWithDetailsDto) {

...
async function createPlan(
  ...
  // Save to db
  const product = await db.portalSubscriptionProducts.createPortalSubscriptionProduct({
    portalId,
  ...
  await Promise.all(
    prices.map(async (price) => {
      ...
      // Save to db
      return await db.portalSubscriptionProducts.createPortalSubscriptionPrice({
        ...price,
        portalId,
  ...
  await Promise.all(
    features
      ...
      .map(async (feature) => {
        // Save to db
        return await db.portalSubscriptionProducts.createPortalSubscriptionFeature(portalId, product.id, feature);
      })
  );
}
...
async function syncPlan(
  ...
  await db.portalSubscriptionProducts.updatePortalSubscriptionProductStripeId(portalId, plan.id, {
    stripeId: stripeProduct.id,
  });
  ...
  prices.map(async (price) => {
    ...
    // Save to db
    await db.portalSubscriptionProducts.updatePortalSubscriptionPriceStripeId(portalId, price.id ?? "", {
      stripeId: stripePrice?.id ?? "",
    });
  });
}
...
async function updatePlan(portalId: string, plan: SubscriptionProductDto, features: SubscriptionFeatureDto[]) {
  ...
  await db.portalSubscriptionProducts.updatePortalSubscriptionProduct(portalId, plan.id, {
    order: plan.order,
    title: plan.title,
    model: plan.model,
    description: plan.description ?? undefined,
  ...
  await db.portalSubscriptionProducts.deletePortalSubscriptionFeatures(portalId, plan.id ?? "");
  ...
  return await Promise.all(
    features
      .sort((a, b) => a.order - b.order)
      .map(async (feature) => {
        return await db.portalSubscriptionProducts.createPortalSubscriptionFeature(portalId, plan.id ?? "", feature);
      })
  );
}
...
async function deletePlan(portalId: string, plan: SubscriptionProductDto) {
  ...
  await Promise.all(
    plan.prices
      ...
      .map(async (price) => {
        ...
        if (price.id) {
          await db.portalSubscriptionProducts.deletePortalSubscriptionPrice(portalId, price.id);
        }
  ...
  if (plan.id) {
    await db.portalSubscriptionProducts.deletePortalSubscriptionProduct(portalId, plan.id);
  }
}

...
export async function load({ request, params, block }: PageBlockLoaderArgs): Promise<RowsOverviewBlockData> {
  const entityName = getValue({ request, params, variable: block?.rowsOverview?.variables?.entityName });
  const tenantId = getValue({ request, params, variable: block?.rowsOverview?.variables?.tenantId });
  const rowId = getValue({ request, params, variable: block?.rowsOverview?.variables?.rowId });

  const userId = (await getUserInfo()).userId;
  const entity = await db.entities.getEntityByName({ tenantId, name: entityName! });
  const rowData = await get(rowId!, {
    entity,
    tenantId,
    userId,
  });

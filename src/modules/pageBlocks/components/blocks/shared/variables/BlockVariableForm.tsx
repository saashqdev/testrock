    ...
    <CollapsibleRow title={name} value={<VariableDescription title={name} item={state} />} initial={false}>
      <div className="grid grid-cols-5 gap-2">
        <InputSelect
          ...
          title="Variable type"
          value={state.type}
          onChange={(value) => setState({ ...state, type: value?.toString() as "manual" | "param" | "query" })}
          options={[
        ...
        {state.type === "param" && (
          <InputRadioGroup
            ...
            title="Param ID"
            value={state.param}
            onChange={(value) => setState({ ...state, param: value as "id1" | "id2" })}
            options={[
              { name: ":id1", value: "id1" },
              { name: ":id2", value: "id2" },
            ]}
        ...
        <InputCheckbox title="Required" name="required" value={state.required} onChange={(value) => setState({ ...state, required: Boolean(value) })} />
      </div>
    </CollapsibleRow>
  );
}
...
function VariableDescription({ title, item }: { title: string; item?: BlockVariableDto }) {
  return (
    <div className="flex items-center space-x-2">
      <div className="shrink-0 font-medium text-foreground">{title}</div>
      {item && (
        <>

...
export default function PageBlockEditMode({
  ...
  return (
    <div>
      {/* {isEditMode() && ( */}
      <div className="bg-gray-900 p-2 text-gray-50 dark:bg-secondary dark:text-foreground">
        <div className="flex justify-center space-x-2">
          <ChatGptSetParametersButton
            page={page}
            className={clsx(
              "shadow-2xs flex items-center justify-center space-x-1 rounded-md border border-transparent px-4 py-2 text-xs font-medium sm:text-sm",
              "border-gray-700 bg-foreground/90 text-gray-100 hover:bg-gray-700"
            )}
            onGenerate={(info) => {
              const newParams = new URLSearchParams(searchParams.toString());
              newParams.set("aiGenerateBlocks", encodeURIComponent(info));

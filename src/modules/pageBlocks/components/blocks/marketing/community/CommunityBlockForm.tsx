    ...
    <div className="space-y-4">
      <InputGroup title="Design">
        <InputRadioGroup
          title="Style"
          value={state.style}
          onChange={(value) => setState({ ...state, style: value as CommunityBlockStyle })}
      ...
      <InputGroup title="Members">
        <div className="space-y-2">
          ...
          <InputCheckboxWithDescription
            ...
            onChange={(e) => setState({ ...state, withName: Boolean(e) })}
          />
          {state.type === "manual" && (
          ...
          {state.cta.map((cta, index) => (
            <div key={index} className="group relative grid grid-cols-2 gap-2">
              <button
                ...
                type="button"
                className="absolute right-0 top-0 -mt-3 hidden origin-top-right justify-center rounded-full bg-background text-muted-foreground hover:text-red-500 group-hover:flex"
              >

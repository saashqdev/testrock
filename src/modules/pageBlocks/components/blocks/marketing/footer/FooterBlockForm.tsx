    ...
    <div className="space-y-4">
      <InputGroup title="Design">
        <InputRadioGroup
          title="Style"
          value={state.style}
          onChange={(value) => setState({ ...state, style: value as FooterBlockStyle })}
    ...
    <CollapsibleRow title={item.name} value={item.name} initial={!item.items || item.items.length === 0} onRemove={onRemove}>
      <div className="space-y-2">
        ...
        <div className="space-y-1">
          <label className="flex justify-between space-x-2 text-xs font-medium text-muted-foreground">Links</label>
          <div className="flex flex-col space-y-2">

      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="xl:grid xl:grid-cols-3 xl:gap-4">
          <div className="space-y-8 xl:col-span-1">
            ...
            {item.text && <p className="text-base text-muted-foreground">{t(item.text)}</p>}
            ...
            {item.sections.map((section, idx) => {
              return (
                <div key={idx} className={clsx(idx > 0 && "mt-0")}>
                  <h3 className="text-sm font-bold uppercase tracking-wider text-foreground dark:text-muted-foreground">{t(section.name)}</h3>

        ...
        <div className="lg:grid lg:grid-cols-2 lg:gap-8">
          ...
          <div className="mt-8 flow-root self-center lg:mt-0">
            <div className="-ml-8 -mt-4 flex flex-wrap justify-between lg:-ml-4">
              {items.map((item, idx) => {
                return (
                  <a key={idx} href={item.href} className="ml-8 mt-4 flex shrink-0 grow justify-center lg:ml-4 lg:grow-0" target="_blank" rel="noreferrer">
                    {!item.srcDark ? (
                      <Image className="h-12" src={item.src} alt={item.alt} />
                    ) : (
                      <>
                        <Image className="h-12 dark:hidden" src={item.src} alt={item.alt} />
                        <Image className="hidden h-12 dark:block" src={item.srcDark} alt={item.alt} />
                      </>
                    )}
                  </a>
                );

...
export default function OnboardingVariantModal({
  ...
  return (
    ...
    >
      <div>
        <div className="flex items-center justify-between space-x-2 border-b border-border px-2 py-2">
        ...
        >
          <div className="relative flex h-full">
            {block.steps.length > 0 && (
              <div className="hidden w-1/2 divide-y-2 divide-border overflow-y-scroll bg-secondary/90 sm:block">
                {block.steps.map((step, idx) => {
                  return (
                    <Fragment key={idx}>
                      <button
                        type="button"
                        className={clsx(
                          "focus:outline-hidden w-full truncate px-3 py-3 text-left text-base font-medium",
                          idx === currentStepIdx ? "bg-background text-foreground" : "border-r-2 border-border text-muted-foreground"
                        )}
                        onClick={() => setStep(idx)}
                      >
                          ...
                          {step.icon && (
                            <>
                              {step.icon.startsWith("<svg") ? (
                                ...
                                <Image className="h-5 w-5" src={step.icon} alt={step.title} />
                              ) : (
                ...
                <div className="h-full border-r-2 border-border p-2"></div>
              </div>
            )}
            {block.steps.length > 0 && (
              <div className={clsx("w-full overflow-y-auto bg-background px-4 pb-4")}>
        ...
        <div className="flex items-center justify-between space-x-2 border-t border-border bg-secondary px-2 py-2">
          <div className="flex items-center space-x-2 truncate px-2 py-2">
            ...
            {block.canBeDismissed && (
              <button
                type="button"
                className="focus:outline-hidden rounded-md px-2 py-2 text-xs text-muted-foreground hover:underline focus:underline"
                onClick={dismiss}
              >
              ...
              <div key={idx} className="p-0.5">
                <button
                  ...
                  className={clsx(
                    ...
                    link.isPrimary ? "bg-theme-700 text-white" : "border border-border bg-secondary/90 text-foreground"
                  )}
                >
                  {t(link.text)}
                </button>
              ...
              <div key={idx}>
                ...
                {input.type === "select" && (
                  <InputSelect
                    ...
                    value={state?.[input.name] ?? ""}
                    onChange={(e) => setState({ ...state, [input.name]: e?.toString() ?? "" })}
                  />
                )}
              </div>
            );

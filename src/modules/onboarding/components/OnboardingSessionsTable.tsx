...
export default function OnboardingSessionsTable({
  ...
  onDelete?: (item: OnboardingSessionWithDetailsDto) => void;
  metadata: OnboardingFilterMetadataDto;
  withOnboarding?: boolean;
}) {
  const { t } = useTranslation();
  const [selected, setSelected] = useState<OnboardingSessionWithDetailsDto>();
  const [headers, setHeaders] = useState<RowHeaderDisplayDto<OnboardingSessionWithDetailsDto>[]>([]);
  const appOrAdminData = useAppOrAdminData();

  useEffect(() => {
    const headers: RowHeaderDisplayDto<OnboardingSessionWithDetailsDto>[] = [
      {
        name: "status",
      ...
      <TableSimple
        ...
        noRecords={
          <div className="p-12 text-center">
            <h3 className="mt-1 text-sm font-medium text-foreground">{t("onboarding.session.empty.title")}</h3>
            <p className="mt-1 text-sm text-muted-foreground">{t("onboarding.session.empty.description")}</p>
          </div>
        }
      />
    </>
...
function SessionModal({
  ...
  return (
    <Modal open={open} setOpen={onClose} size="2xl">
      <div className="inline-block w-full overflow-hidden bg-background text-left align-bottom sm:align-middle">
        <div className="flex justify-between space-x-2">
          <h3 className="text-lg font-medium leading-6 text-foreground">Session</h3>
          <div className="text-sm italic text-muted-foreground">{item?.id}</div>
        </div>

...
export default function ApiKeyLogsSummary({ data }: Props) {
  ...
  const [groupBy, setGroupBy] = useState<string[]>(ApiKeyLogUtils.getGroupByValues(newSearchParams));
  const router = useRouter();

  const [headers, setHeaders] = useState<RowHeaderDisplayDto<ApiCallSummaryDto>[]>([]);

  useEffect(() => {
    if (ApiKeyLogUtils.getGroupByValues(newSearchParams).sort().join(",") !== groupBy.sort().join(",")) {

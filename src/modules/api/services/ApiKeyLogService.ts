...
async function getSummary({ request, params }: { request: Request; params: Params }) {
  ...
  if (groupBy.length > 0) {
    try {
      const data = await prisma.apiKeyLog.groupBy({
...
async function getGroupBys(tenantId: string | null) {
  ...
  // const allEndpoints = await prisma.apiKeyLog.groupBy({
  //   by: ["endpoint"],
  //   where,
  //   _count: { endpoint: true },
  //   orderBy: { _count: { endpoint: "desc" } },
  // });
  // const allParams = await prisma.apiKeyLog.groupBy({
  //   where,
  //   by: ["params"],
  //   _count: { params: true },
  //   orderBy: { _count: { params: "desc" } },
  // });
  // const allStatus = await prisma.apiKeyLog.groupBy({
  //   by: ["status"],
  //   where,
  //   _count: { status: true },
...
async function getFilterableProperties(tenantId: string | null) {
  ...
  if (tenantId === null) {
    allTenants = await db.tenants.adminGetAllTenantsIdsAndNames();
...
async function deleteMany(ids: string[]) {
  return await prisma.apiKeyLog.deleteMany({
    where: { id: { in: ids } },
  });
}

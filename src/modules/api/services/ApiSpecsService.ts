...
async function generateSpecs({ request }: { request: Request }): Promise<ApiSpecsDto> {
  ...
  let entities = (await db.entities.getAllEntities(null)).filter((f) => f.hasApi);
...
function getEntityEndpoints({ entity, t }: { entity: EntityWithDetailsDto; t: TFunction }) {
...
function generateOpenApiSpecs({ entities, t }: { entities: EntityWithDetailsDto[]; t: TFunction }) {
  let paths: any = {};
...
function generateEntitySwaggerSpec({ entity, properties, t }: { entity: EntityWithDetailsDto; properties: any; t: TFunction }) {
  const responses = {
    "200": { description: `OK` },
    "400": { description: `Bad Request` },
    "404": { description: `Not Found` },

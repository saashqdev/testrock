...
export const NodeSelector: React.FC<NodeSelectorProps> = ({ editor, isOpen, setIsOpen }) => {
  const items: BubbleMenuItem[] = [
    ...
    },
    {
      name: "Heading 1",
      icon: Heading1,
      command: () => editor.chain().focus().toggleNode("heading", "heading").run(),
      isActive: () => editor.isActive("heading", { level: 1 }),
    },
    {
      name: "Heading 2",
      icon: Heading2,
      command: () => editor.chain().focus().toggleNode("heading", "heading", { level: 2 }).run(),
      isActive: () => editor.isActive("heading", { level: 2 }),
    },
    {
      name: "Heading 3",
      icon: Heading3,
      command: () => editor.chain().focus().toggleNode("heading", "heading", { level: 3 }).run(),
      isActive: () => editor.isActive("heading", { level: 3 }),
    },
    {
      name: "List (•)",
      icon: ListOrdered,
      command: () => editor.chain().focus().toggleList("bulletList", "listItem").run(),
      isActive: () => editor.isActive("bulletList"),
    },
    {
      name: "List (1.)",
      icon: ListOrdered,
      command: () => editor.chain().focus().toggleList("orderedList", "listItem").run(),
    ...
    },
    {
      name: "Code",
      icon: Code,
      command: () => editor.chain().focus().toggleNode("codeBlock", "paragraph").run(),

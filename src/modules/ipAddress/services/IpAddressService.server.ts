...
async function log(
  ...
  let blacklistedIp = await db.blacklist.findInBlacklist("ip", ip);
  if (!blacklistedIp && block) {
    blacklistedIp = await db.blacklist.addToBlacklist({ type: "ip", value: ip });
  }
  if (blacklistedIp) {
    await db.blacklist.addBlacklistAttempt(blacklistedIp);
    await prisma.ipAddressLog.create({
      data: {
...
async function getOrCreateIpAddressLookup(ip: string): Promise<IpAddressDto | null> {
  ...
  const createdIp = await prisma.ipAddress
    .create({
      data: {
        ip,

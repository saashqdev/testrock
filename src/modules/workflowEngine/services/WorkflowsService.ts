...
async function create(session: { tenantId: string | null; userId: string | null }) {
  ...
  return await db.workflows.createWorkflow({
...
async function get(id: string, { tenantId }: { tenantId: string | null }): Promise<WorkflowDto | null> {
  const item = await db.workflows.getWorkflowById({ id, tenantId });
  if (!item) {
    return null;
  }
  const $variables = await db.workflowVariable.getAllWorkflowVariables({ tenantId });
  const $credentials = await db.workflowCredentials.getAllWorkflowCredentials({ tenantId });
...
async function getById(id: string, { tenantId }: { tenantId: string | null }): Promise<WorkflowDto | null> {
  const item = await db.workflows.getWorkflow(id);
  if (!item) {
    return null;
  }
  const $variables = await db.workflowVariable.getAllWorkflowVariables({ tenantId });
...
async function getAll({ tenantId }: { tenantId: string | null }): Promise<WorkflowDto[]> {
  const items = await db.workflows.getAllWorkflows({ tenantId });
...
async function getAllAppliesToAllTenants({ tenantId }: { tenantId: string | null }): Promise<WorkflowDto[]> {
  const items = await db.workflows.getAllWorkflowsAppliesToAllTenants({ tenantId, status: "live" });
  return await Promise.all(items.map((item) => workflowToDto(item)));
}

async function workflowToDto(
  item: WorkflowWithDetailsDto,
  global?: {
    $variables: string[];
    $credentials: string[];
  }
...
async function addConditionGroup(workflowBlock: WorkflowBlockDto, data: WorkflowConditionsGroupDto[]) {
  return await Promise.all(
    data.map(async (conditionGroup) => {
      ...
      await prisma.workflowBlockConditionGroup.create({
        data: {
...
async function update(
  ...
  await db.workflows.updateWorkflow(id, {
    name: workflow.name,
...
async function getNextUntiledWorkflowName({ tenantId }: { tenantId: string | null }) {
  const allWorkflowNames = await prisma.workflow.findMany({
    where: {
      tenantId,
    },
    select: { name: true },
...
async function del(id: string, session: { tenantId: string | null }) {
  ...
  return await db.workflows.deleteWorkflow(id);
}

async function updateBlock(id: string, block: WorkflowBlockDto, { workflow }: { workflow: WorkflowDto }) {
  const item = await db.workflowBlocks.updateWorkflowBlock(id, {
    type: block.type,
  ...
  if (block.type === "event" && block.input.event) {
    if (workflow.name.startsWith("Untitled workflow")) {
      await db.workflows.updateWorkflow(item.workflowId, {
...
async function addBlock({
  ...
  const toBlock = await db.workflowBlocks.createWorkflowBlock({
  ...
  if (fromBlockId) {
    await prisma.workflowBlockToBlock.create({
      data: {
        fromBlockId,
...
async function updateWorkflowApplyToAllTenants(workflowId: string) {
  const blocks = await db.workflowBlocks.getWorkflowBlockTypes(workflowId);
  ...
  const item = await db.workflows.updateWorkflow(workflowId, {
    appliesToAllTenants,
  });
  if (item.tenantId && item.appliesToAllTenants) {
    // just for good measure
    await db.workflows.updateWorkflow(workflowId, {
...
async function deleteBlock(id: string, { workflow }: { workflow: WorkflowDto }) {
  const item = await prisma.workflowBlock.delete({
    where: { id },
  });
  if (workflow.tenantId === null) {
    await updateWorkflowApplyToAllTenants(item.workflowId);
...
async function connectBlocks({ fromBlockId, toBlockId, condition }: { fromBlockId: string; toBlockId: string; condition: string | null }) {
  return await prisma.workflowBlockToBlock.create({
    data: {
      fromBlockId,
      toBlockId,
      condition,
...
async function deleteConnection(params: { fromBlockId: string; toBlockId: string } | { id: string }) {
  if ("id" in params) {
    return await prisma.workflowBlockToBlock.delete({
      where: { id: params.id },
    });
  } else {
    return await prisma.workflowBlockToBlock.deleteMany({
      where: {
...
async function updateConditionsGroups(block: WorkflowBlockDto, conditionGroups: WorkflowConditionsGroupDto[]) {
  await prisma.workflowBlockConditionGroup.deleteMany({
    where: {
...
async function getExecutions(item: WorkflowDto, { tenantId }: { tenantId: string | null }) {
  const items = await db.workflowExecutions.getWorkflowExecutions({ workflowId: item.id }, { tenantId });
  return items.map((f) => WorkflowExecutionUtils.rowToDto(f));
}

async function getExecution(id: string, { tenantId }: { tenantId: string | null }) {
  const item = await db.workflowExecutions.getWorkflowExecution(id, { tenantId });
  if (!item) {
    return null;
  }
...
async function createInputExample({ workflowId, title, input }: { workflowId: string; title: string; input: string }) {
  return await prisma.workflowInputExample.create({
    data: {
      workflowId,
      title,
      input,
...
async function updateInputExample(id: string, { title, input }: { title: string; input: string }) {
  return await prisma.workflowInputExample.update({
    where: { id },
    data: {
      title,
      input,
...
async function deleteInputExample(id: string) {
  return await prisma.workflowInputExample.delete({
    where: { id },
  });
}
export default {

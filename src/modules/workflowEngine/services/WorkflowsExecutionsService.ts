...
async function execute(
  ...
  let tenant = session.tenantId
    ? await prisma.tenant.findFirstOrThrow({ where: { OR: [{ slug: session.tenantId }, { id: session.tenantId }] } }).catch(() => null)
    : null;
  ...
  const updatedExecution = await db.workflowExecutions.updateWorkflowExecution(execution.id, {
...
async function continueExecution(
  ...
  const execution = await db.workflowExecutions.getWorkflowExecution(executionId, session);
  if (!execution) {

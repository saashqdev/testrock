...
async function execute({
  ...
  let blockExecution = await prisma.workflowBlockExecution.create({
    data: {
      workflowExecutionId,
  ...
  await prisma.workflowBlockExecution.update({
  ...
  const updatedExecution = await db.workflowExecutions.getWorkflowExecution(workflowExecutionId, session);
  if (updatedExecution) {

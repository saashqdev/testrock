  ...
  return (
    <button
      ...
      className={clsx(
        ...
        !workflowBlockExecution && "border-border bg-background focus:ring-blue-600",
      ...
      <div className="w-full space-y-2">
        <div className="flex justify-between space-x-2">
          <div className="flex items-center space-x-2 truncate">
            ...
            <div className="truncate text-sm font-medium text-foreground/80">{workflowBlock.name}</div>
          </div>
          <div>
            <div
              className={clsx(
                "rounded-md border px-2 py-1 text-xs",
                workflowBlock.type === "trigger" ? "border-yellow-200 bg-yellow-50 text-yellow-800" : "border-border bg-secondary text-muted-foreground"
              )}
            >
        ...
        {errors.length > 0 ? (
          ...
          <div className="line-clamp-1 text-xs text-muted-foreground">{block.description || block.input.event || "No description"}</div>
        )}

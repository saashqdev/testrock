...
async function getSummary(tenantId: string | null): Promise<EmailMarketingSummaryDto> {
  const allOutboundEmails = await db.outboundEmails.getAllOutboundEmails(tenantId, {
...
async function sendPreview({
  ...
  try {
    ...
    await sendBroadcast({
      ...
      track: email.track,
      metadata: { outboundEmailId: outboundEmail.id },
    });
...
async function sendContactPreview({
  ...
  const outboundEmail = await db.outboundEmails.createOutboundEmail({
    tenantId: from.tenantId,
    contactRowId,
    email: email.to,
    fromSenderId: from.sender.id,
...
async function createCampaignDraft({
  ...
  const campaign = await db.campaigns.createCampaign(
    {
...
async function sendCampaignTest(campaign: CampaignWithDetailsDto, email: string): Promise<boolean> {
...
async function sendCampaign(campaign: CampaignWithDetailsDto): Promise<boolean> {
  if (campaign.recipients.length === 0) {
    throw new Error("No recipients found");
  }
  ...
  await db.campaigns.updateCampaign(campaign.id, {
    status: "sending",
  });
  let totalSent = 0;
  await Promise.all(
    campaign.recipients.map(async (recipient) => {
      try {
        ...
        await db.outboundEmails.updateOutboundEmail(recipient.id, {
          sentAt: new Date(),
          error: undefined,
        });
        await sendBroadcast({

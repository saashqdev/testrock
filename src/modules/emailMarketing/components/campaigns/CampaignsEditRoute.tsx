...
export default function CampaignsEditRoute({ data, actionData, onSubmit }: CampaignsEditRouteProps) {
  ...
  function sendTest(i?: RowWithDetailsDto) {
    ...
    if (onSubmit) {
      setIsSubmitting(true);
      onSubmit(form).finally(() => setIsSubmitting(false));
    }
  }
  ...
  function filteredRecipients() {
    ...
    return emails.filter((f: any) => f.email?.toString().toUpperCase().includes(searchInput.toUpperCase()));
  }
  return (
    <EditPageLayout
            ...
            {data.item.status === "draft" && (
              <div className="flex items-center justify-between">
                ...
                <div className="flex justify-between space-x-2">
                  <ButtonSecondary disabled={isSubmitting} onClick={() => sendTest()} className="truncate">
                    {data.item.recipients.length === 1 ? t("emailMarketing.sendPreview") : t("emailMarketing.sendPreview", { 0: data.item.recipients.length })}
                  </ButtonSecondary>
                  <ButtonPrimary disabled={isSubmitting} onClick={send} className="truncate">
